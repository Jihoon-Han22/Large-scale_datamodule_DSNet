...
def seq2bbox(sequence: np.ndarray) -> np.ndarray:
    ...
    B, _= np.array(sequence).shape
    result_bboxes_lr = []
    for b in range(B):
        in_batch_sequence = np.asarray(sequence[b], dtype=np.bool)
        selected_indices, = np.where(in_batch_sequence == 1)
        ...
        result_bboxes_lr.append(bboxes_lr)
    return result_bboxes_lr
...
def nms(scores,
    ...
    arg_desc = scores.squeeze(1).argsort()[::-1]
    # print("arg_desc")
    # print(arg_desc)
