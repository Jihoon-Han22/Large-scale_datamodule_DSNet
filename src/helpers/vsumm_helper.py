...
def get_keyshot_summ(pred: np.ndarray,
    ...
    for b in range(B):
        ...
        # frame_scores = np.zeros(n_frames[b], dtype=np.float32)
        # for i in range(len(picks)):
        #     pos_lo = picks[b][i]
        #     pos_hi = picks[b][i + 1] if i + 1 < (picks[b]).size else n_frames[b]
        #     print("(picks[b]).size")
        #     print((picks[b]).size)
        #     frame_scores[pos_lo:pos_hi] = pred[b][i]
        #     print("frame_scores")
        #     print(frame_scores)
        ...
        summary = np.zeros(n_frames[b], dtype=np.bool)
